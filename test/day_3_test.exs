defmodule DayThreeTest do
  ...
  test "Part One example 1" do
    ...
    paths = paths
    |> Enum.map(&DayThree.find_wire_points/1)
    intersections = DayThree.find_intersections(paths)
    answer = DayThree.find_manhattan_distance_of_closest_intersection(intersections)
    assert answer == 6
  end
